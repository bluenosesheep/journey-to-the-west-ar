...
AFRAME.registerComponent("park-canvas",{
  init:function(){
    ...
    // Magic Demo visual branch only: formal bench PNG.
    ...
    this.flowerImage.src="./assets/park/flower.png?v=1";
  ...
  tick:function(){
    ...
    Object.entries(this.items).forEach(([kind,item])=>{
      ...
      if(
        ...
        this.benchImage.naturalWidth>0
      ){
        const bw=118;
        const bh=118;
        ...
        kind==="flower" &&
        this.flowerImage &&
        this.flowerImage.complete &&
        this.flowerImage.naturalWidth>0
      ){
        const flw=110;
        const flh=110;
        ctx.drawImage(this.flowerImage,-flw/2,-flh/2,flw,flh);
      }else{
